...
public class Python3Generator implements CodeGenerator
{
    ...
    public Python3Generator(final Ir ir, final OutputManager outputManager)
    {
    ...
    private void generateGroups(final StringBuilder sb, final List<Token> tokens, final String indent)
    {
        ...
        for (int i = 0, size = tokens.size(); i < size; i++)
        {
            ...
            // below is almost same code for every languages (cpp, golang, python and etc ...)
        ...
        for (int i = 0, size = tokens.size(); i < size; i++)
        {
            ...
            if (token.signal() == Signal.BEGIN_VAR_DATA)
            {
                ...
                sb.append(String.format(
                    ...
                    "        struct.pack_into(fmt, self.buffer_, pos, str(buffer).encode())\n" +
        ...
        for (final Token token : tokens)
        {
            if (token.signal() == Signal.CHOICE)
            {
                ...
                sb.append(String.format(
                    ...
                    "'%4$s', self.buffer_, self.offset_)[0] & (0x1 << %5$s) > 0 else False\n\n",
                ...
                sb.append(String.format(
                    ...
                    "        bits = (bits | ( 0x1 << %5$s)) if value > 0 else (bits & ~(0x1 << %5$s))\n" +
        ...
        final String propertyName, final Token token, final String indent)
    {
        ...
        if (token.encoding().primitiveType() == PrimitiveType.CHAR)
        {
            sb.append(String.format(
                    indent + "    def get%2$s(self):\n" +
                    indent + "        return bytes(struct.unpack_from('%1$s%4$ds', self.buffer_, self.offset_ + %3$d)[0]).decode()\n\n",
        ...
        if (token.encoding().primitiveType() == PrimitiveType.CHAR)
        {
            sb.append(String.format(
                    indent + "    def set%2$s(self, value):\n" +
                            ...
                            indent + "            raise Exception('%2$s length not %3$d')\n" +
                            indent + "        struct.pack_into('%1$s%3$ds', self.buffer_, self.offset_ + %4$d, str(value).encode())\n",
                    byteOrderFormat(token.encoding().byteOrder()),
                    toUpperFirstChar(propertyName),
                    token.arrayLength(),
                    offset
        ...
        final String containingClassName, final String propertyName, final Token token, final String indent)
    {
        ...
        if (token.encoding().primitiveType() == PrimitiveType.CHAR) {
            sb.append(String.format(
                "\n" +
                indent + "    def get%2$s(self):\n" +
                indent + "        return %1$s.%1$s(bytes(struct.unpack_from( '%5$s', self.buffer_, self.offset_ + %6$d)[0]).decode())\n\n",
                enumName,
                toUpperFirstChar(propertyName),
        ...
        // python3 '<c' should require bytes, not str(unicode)
        if (token.encoding().primitiveType() == PrimitiveType.CHAR) {
            sb.append(String.format(
                indent + "    def set%2$s(self, value):\n" +
                indent + "        struct.pack_into('%4$s', self.buffer_, self.offset_ + %5$d, str(value).encode())\n" +
                indent + "        return self\n",
                formatClassName(containingClassName),
                toUpperFirstChar(propertyName),
                enumName,
